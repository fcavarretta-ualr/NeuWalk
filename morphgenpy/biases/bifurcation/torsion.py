...
def radial_torsion_bias(rng, neurite, angle):
    repulsion = misc._normalize(elongation.all_dendrites_repulsion(rng, neurite, neurite.last_direction, None, None))

    if repulsion:
      directions = _torsion_bias(rng, neurite, angle, repulsion)
      if directions:
        return directions
