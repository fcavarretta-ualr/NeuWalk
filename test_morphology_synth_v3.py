...
def main():
    ...
    for i, m in enumerate(load_morphologies(args.directory, root_section_types="apical_dendrite", delete_section_types=["unknown", "apical_secondary_oblique", "apical_secondary_dendrite"])):
      if i == 2:
        print(m['filename'])
    
    print()
    basal_dendrites = [extract_neurites(m['morphology'], section_type="basal_dendrite") for m in load_morphologies(args.directory, root_section_types="basal_dendrite") if len(m['morphology'])]
    apical_dendrites = [extract_neurites(m['morphology'], section_type="apical_dendrite") for m in load_morphologies(args.directory, root_section_types="apical_dendrite", delete_section_types=["unknown", "apical_oblique", "apical_secondary_oblique", "apical_secondary_dendrite"]) if len(m['morphology'])]
    apical_dendrites_with_oblique = [extract_neurites(m['morphology'], section_type="apical_dendrite") for m in load_morphologies(args.directory, root_section_types="apical_dendrite", delete_section_types=["unknown", "apical_secondary_oblique", "apical_secondary_dendrite"]) if len(m['morphology'])]
    apical_obliques = [extract_neurites(m['morphology'], section_type="apical_oblique") for m in load_morphologies(args.directory, root_section_types="apical_oblique") if len(m['morphology'])]
    ...
    stats['apical_dendrites_with_oblique'] = extract_statistics(
      apical_dendrites_with_oblique,
      bin_size=args.bin_size
      )
