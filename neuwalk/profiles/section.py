import numpy as np
from ..core.section_object import SectionObject, TYPE_LABELS, TYPE_CODES


class SectionProfile(SectionObject):
    """
    ...
    def create_primary_sections(self, number, label):
        ...
        for _ in range(number):
            self.connect(
                SectionProfile(self.step_size, label=label),
                relation="child")
    ...
    def bifurcate(self):
        ...
        for _ in range(2):
            self.connect(
                SectionProfile(self.step_size, label=self.label),
                relation="child"
                )
